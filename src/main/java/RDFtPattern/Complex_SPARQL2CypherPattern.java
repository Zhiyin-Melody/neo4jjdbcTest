...
public class Complex_SPARQL2CypherPattern {
        ...
        if(SPARQL_Pattern1.contains("OPTIONAL")){
            ...
            for (int i = 0; i <rdfgrapgStr.length; i++) {
                ...
                if(i==rdfgrapgStr.length-1){
                    ...
                    cypher_graphStr.append(BaseRDFPattern2Cypher.BaseRDFPattern2Cypher(str)).append(",");
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        //可选图模式；
        String SPARQL_Pattern1="{S1 ?p ?O1 . ?p rdf:type rdft:property . ?p rdft:hasStartTime ts1^^xsd:date . ?p rdft:hasEndTime te1^^xsd:date . ?p rdft:hasNumUpdate 14 .}\n" +
                " OPTIONAL {S2 ?p ?O2 . ?p rdf:type rdft:property . ?p rdft:hasStartTime ts2^^xsd:date . ?p rdft:hasEndTime te2^^xsd:date . ?p rdft:hasNumUpdate 14 .}\n";
        //多图模式；
        ...
        System.out.println(Complex_SPARQL2CypherPattern(SPARQL_Pattern2));
        System.out.println(Complex_SPARQL2CypherPattern(SPARQL_Pattern3));
